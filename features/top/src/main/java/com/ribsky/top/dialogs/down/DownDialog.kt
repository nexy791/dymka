package com.ribsky.top.dialogs.down
...
import com.ribsky.top.databinding.DialogDownBinding
...
class DownDialog(
...
) : BaseSheet<DialogDownBinding>(DialogDownBinding::inflate) {
    ...
    override fun initViews() {
        ...
        type.let {
            Analytics.logEvent(
                when (it) {
                    TopDialogsNavigation.Type.STARS -> Analytics.Event.TOP_DOWN_STARS
                    TopDialogsNavigation.Type.TESTS -> Analytics.Event.TOP_DOWN_TESTS
                }
            )
        }
    }
    ...
    private fun initAnim() = with(binding) {
        root.postDelayed({
            AnimExt.swapAnimation(layoutTopView, layoutTopView4)
            vibrate()
            playSound(commonRaw.sound_down)
        }, 1000)
    ...
    private fun updateUi(list: List<TopDialogsNavigation.UserModel>) = with(binding) {
        ...
        list.forEachIndexed { index, baseTopModel ->
            when (index) {
                ...
                1 -> updateUiView(layoutTopView4, baseTopModel)
    ...
    companion object {
        ...
        fun newInstance(
            ...
            DownDialog(type, users, onDismiss)
    }
}
